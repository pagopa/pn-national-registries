...
def signing_algorithm(hashalgo, signalgo):
    ...
    if hashalgo == 'sha512' and signalgo == 'ECDSA':
        ...
        raise ValueError('unknown hash algorithm, please specify one of sha224, sha256, sha384, or sha512')
...
def main(args):
    ...
    # now get the key
    if not args.region:
        args.region = boto3.Session().region_name
    ...
    sig_alg_identifier.setComponentByName('algorithm',
