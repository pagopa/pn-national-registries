...
public class InfocamereClientConfig extends CommonBaseClient {
    ...
    protected ExchangeFilterFunction buildRetryExchangeFilterFunction() {
        return (request, next) -> {
            ...
            }).retryWhen(Retry.backoff((long) customRetryConfig.getRetryMaxAttempts(), Duration.ofMillis(25L)).jitter(0.75).filter(throwable ->  CustomRetryConfig.isRetryableException(throwable, MaskTaxIdInPathUtils.maskTaxIdInPathICRegistroImprese(throwable.getMessage()))).onRetryExhaustedThrow((retryBackoffSpec, retrySignal) -> {
